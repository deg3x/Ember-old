...
void main()
{
    ...
    const float thicknessBold = 1.5 * thicknessNorm;
    ...
    // Grid Test Bold
    derivative = fwidth(WorldPos.xz / frequency);
    gridAA     = derivative * 1.5;
    gridUV     = 1.0 - abs(fract(WorldPos.xz / frequency) * 2.0 - 1.0);
    ...
    // Color Test
    vec3 colorOutput = step(gridBold, gridNorm) * colorDefault + step(gridNorm, gridBold) * colorBold;
